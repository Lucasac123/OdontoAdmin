  ...
  return (
    <div className={`flex items-center ${iconOnly ? '' : 'gap-3'} ${className}`}>
      <div className="relative group">
        <div className="absolute -inset-1.5 bg-gradient-to-tr from-indigo-600 via-indigo-400 to-indigo-300 rounded-2xl blur opacity-20 group-hover:opacity-40 transition duration-500"></div>
      ...
      {!iconOnly && (
        <div className={`flex flex-col ${isVertical ? 'items-center -space-y-1' : '-space-y-1'}`}>
          <span className={`${isVertical ? 'text-3xl' : 'text-xl'} font-extrabold tracking-tight text-text-primary`}>
            Odonto<span className="bg-gradient-to-r from-indigo-600 to-indigo-400 bg-clip-text text-transparent">Admin</span>
          </span>
